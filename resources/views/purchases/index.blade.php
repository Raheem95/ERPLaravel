            ...
            @foreach ($Purchases as $Purchase)
                <tbody>
                    ...
                    <tr>
                        ...
                        <td id = "TotalPurchase{{ $Purchase->PurchaseID }}">{{ $Purchase->TotalPurchase }}
                        </td>
                        <td data-toggle="modal" data-target="#PaymentDetailsModel"
                            id = "PaidAmount{{ $Purchase->PurchaseID }}"
                            onclick="viewPaymentDetails({{ $Purchase->PurchaseID }})">
                            {{ $Purchase->PaidAmount }}</td>
                        ...
                        <td>
                            @if ($Purchase->Transfer == 0)
                                <button class="btn view_button"><i class="fa-solid fa-shuffle fa-2x"></i></button>
                        </td>
                    @else
                        -
            @endif
...
@else
    ...
    <script>
        ...
        $(document).on('click', '.SavePayment', function() {
            ...
            var TotalPurchase = parseFloat($("#TotalPurchase" + PurchaseID).html())
            var PaidAmount = parseFloat($("#PaidAmount" + PurchaseID).html())
                            ...
                            success: function(result) {
                                if (result == 1) {
                                    ...
                                    $("#Results").removeClass("alert-danger").addClass("alert-success")
                                        .html(
                                            "تم الدفع بنجاح");
                                    PaidAmount = parseFloat($("#PaidAmount" + PurchaseID).html()) +
                                        Amount;
                                    $("#PaidAmount" + PurchaseID).html(PaidAmount)
                                    if (TotalPurchase == PaidAmount)
            ...
            $.ajax({
                ...
                success: function(response) {
                    for (var i = 0; i < response.length; i++) {
                        ...
                        tr.append($("<td>" + response[i]["PaidAmount"] + "</td>"))
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (!isNaN(result)) {
                            ...
                            $("#PaidAmount" + PurchaseID).html(parseFloat($("#PaidAmount" +
                                    PurchaseID)
                                .html()) - parseFloat(result))
                            $("#DeletePaymentResults").removeClass("alert-danger").addClass(
                                    "alert-success")
                                .html(
                                    "تم حذف السداد بنجاح");
                            $("#PaymentNo" + PaymentID).remove()
                            if ($("#PaidAmount" + PurchaseID).html() == 0) {
